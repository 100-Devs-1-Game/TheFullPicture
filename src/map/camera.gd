...
# ZOOM_IN_3X removed entirely

var _map: Node2D = null
var _zoom_factors: Array[float] = []
var _zoom_level: int = 1  # 0 = fit, 1 = 1×, 2 = 2×
...
func _ready() -> void:
	...
	# Only three levels now: fit → 1× → 2×
	_zoom_factors = [fit_zoom, ZOOM_DEFAULT, ZOOM_IN_2X]

	global_position = _map.map_size * 0.5
	_set_zoom_level(1)  # start at 1×
...
func _process(delta: float) -> void:
	...
	if mouse_pos.x < edge_pan_margin: edge_dir.x -= 1
	if mouse_pos.x > vp_rect.size.x - edge_pan_margin: edge_dir.x += 1
	if mouse_pos.y < edge_pan_margin: edge_dir.y -= 1
	if mouse_pos.y > vp_rect.size.y - edge_pan_margin: edge_dir.y += 1
...
func _set_zoom_level(idx: int, center_on_mouse: bool = true) -> void:
	...
	var new_zoom_value := _zoom_factors[idx]
	var target_zoom := Vector2.ONE * new_zoom_value
...
func _change_zoom_level(delta: int) -> void:
	_set_zoom_level(_zoom_level + delta, true)
...
func _clamp_to_map_bounds() -> void:
	...
	var max_pos := map_size - half_visible
...
func _find_map_node(node: Node) -> Node:
	if node.has_method("get_map_size") or "map_size" in node:
		return node
